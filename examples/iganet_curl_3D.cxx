...
public:
  ...
  bool epoch(int64_t epoch) override {
    ...
    if (epoch == 0) {
      ...
      G_knot_indices_ =
          Base::G_.template find_knot_indices<iganet::functionspace::interior>(std::get<0>(
              collPts_.first));
      G_coeff_indices_ =
  ...
  torch::Tensor t_ana1() {
      ...
      auto Geo = Base::G_.eval(std::get<0>(collPts_.first), G_knot_indices_, G_coeff_indices_);
      auto x = *Geo[0];
      auto y = *Geo[1];
      auto z = *Geo[2];
  ...
  torch::Tensor t_ana2() {
      ...
      auto Geo = Base::G_.eval(std::get<1>(collPts_.first), G_knot_indices_, G_coeff_indices_);
      auto x = *Geo[0];
      auto y = *Geo[1];
      auto z = *Geo[2];
  ...
  torch::Tensor t_ana3() {
      ...
      auto Geo = Base::G_.eval(std::get<2>(collPts_.first), G_knot_indices_, G_coeff_indices_);
      auto x = *Geo[0];
      auto y = *Geo[1];
      auto z = *Geo[2];
...
int main() {
  ...
  using variable_t = iganet::Hcurl<iganet::UniformBSpline<real_t, 1, 2, 2, 2>>;
  ...
  net.ref().template space<0>().transform([](const std::array<real_t, 3> xi) {
      return std::array<real_t, 1>{M_PI* (2.0 * sin(2.0 * M_PI * xi[0]) * cos(2.0 * M_PI * xi[1]) - sin(2.0 * M_PI * xi[0]) * cos(M_PI * xi[2]))};
      });
  net.ref().template space<1>().transform([](const std::array<real_t, 3> xi) {
      return std::array<real_t, 1>{
          M_PI * (-2.0 * sin(2.0 * M_PI * xi[1]) * cos(2.0 * M_PI * xi[0]) + sin(2.0 * M_PI * xi[1]) * cos(M_PI * xi[2]))};
      });
  net.ref().template space<2>().transform([](const std::array<real_t, 3> xi) {
      return std::array<real_t, 1>{
  ...
  net.ref().template boundary<1>().template side<1>().transform(
      [](const std::array<real_t, 2> xi) {
          return std::array<real_t, 1>{0.0};
      });
  net.ref().template boundary<2>().template side<1>().transform(
      [](const std::array<real_t, 2> xi) {
          return std::array<real_t, 1>{0.0};
      });
  net.ref().template boundary<0>().template side<2>().transform(
      [](const std::array<real_t, 2> xi) {
          return std::array<real_t, 1>{0.0};
      });
  net.ref().template boundary<1>().template side<2>().transform(
      [](const std::array<real_t, 2> xi) {
          return std::array<real_t, 1>{0.0};
      });
  net.ref().template boundary<2>().template side<2>().transform(
      [](const std::array<real_t, 2> xi) {
          return std::array<real_t, 1>{0.0};
      });
  net.ref().template boundary<0>().template side<3>().transform(
      [](const std::array<real_t, 2> xi) {
          return std::array<real_t, 1>{0.0};
      });
  net.ref().template boundary<1>().template side<3>().transform(
      [](const std::array<real_t, 2> xi) {
          return std::array<real_t, 1>{0.0};
      });
  net.ref().template boundary<2>().template side<3>().transform(
      [](const std::array<real_t, 2> xi) {
          return std::array<real_t, 1>{0.0};
      });
  net.ref().template boundary<0>().template side<4>().transform(
      [](const std::array<real_t, 2> xi) {
          return std::array<real_t, 1>{0.0};
      });
  net.ref().template boundary<1>().template side<4>().transform(
      [](const std::array<real_t, 2> xi) {
          return std::array<real_t, 1>{0.0};
      });
  net.ref().template boundary<2>().template side<4>().transform(
      [](const std::array<real_t, 2> xi) {
          return std::array<real_t, 1>{0.0};
      });
  net.ref().template boundary<0>().template side<5>().transform(
      [](const std::array<real_t, 2> xi) {
          return std::array<real_t, 1>{0.0};
      });
  net.ref().template boundary<1>().template side<5>().transform(
      [](const std::array<real_t, 2> xi) {
          return std::array<real_t, 1>{0.0};
      });
  net.ref().template boundary<2>().template side<5>().transform(
      [](const std::array<real_t, 2> xi) {
          return std::array<real_t, 1>{0.0};
      });
  ...
  net.ref().template boundary<1>().template side<6>().transform(
      [](const std::array<real_t, 2> xi) {
          return std::array<real_t, 1>{0.0 };
      });
  net.ref().template boundary<2>().template side<6>().transform(
      [](const std::array<real_t, 2> xi) {
          return std::array<real_t, 1>{0.0};
      });
